...
<title>购物车</title>
	...
	<script type="text/javascript">
		$(function () {
			...
			return 	confirm("确定要删除"+$(this).parent().parent().find(":first").text()+"吗?")
			...
			if(confirm("你确定要修改["+name+"]吗!数量为"+count)){
	...
	<div id="main">
		...
		<table>
			...
			<c:if test="${ empty sessionScope.cart.items}">
				<tr>
					<td colspan="5"><a href="index.jsp">亲,购物车为空.</a></td>
			...
			<c:if test="${not empty sessionScope.cart.items}">
				<c:forEach items="${sessionScope.cart.items}" var="item">
					<tr>
						...
						<td>
						<input class="updateCount"  style="width: 50px" type="text"  bookId="${item.value.id}" value="${item.value.count}">
						</td>
